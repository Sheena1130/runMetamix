...
parser$add_argument('--type', default = 'prot', help = 'prot or nucl')
...
parser$add_argument('--poster_prob_thr', type = 'double', default = 0.9, help = 'posterior probability of presence of species threshold for reporting in the species summary')
parser$add_argument('--threads', default = 12, type = 'integer', help = 'number of threads')
...
if (args$type == 'prot') {
   step3 <- parallel.temper(step2 = step2,
                            readSupport = args$read_support,
                            noChains = args$threads)
} else {
   step3 <- parallel.temper.nucl(step2 = step2,
                                 readSupport = args$read_support,
                                 noChains = args$threads)
}
...
# save file in txt
write.table(step4$presentSpecies.allInfo,
            file = args$output,
            quote = FALSE,
            sep = '\t',
            row.names = FALSE)
